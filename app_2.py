...
# ------------------------------
st.set_page_config(page_title="LSTM‑Attention Stock Predictor", layout="wide")
...
def load_trained_model(ticker: str):
    ...
    model = LSTM_Attention_LSTM(input_dim=1, hidden_dim=100, output_dim=1).to(DEVICE)
...
st.title("📈 Dự đoán giá cổ phiếu bằng mô hình LSTM‑Attention‑LSTM")
st.markdown(
    "Sử dụng mô hình được huấn luyện riêng cho từng mã cổ phiếu để dự báo xu hướng giá và hiển thị **EMA** 20/50/100/200 ngày."
)
# ------------------------------
if run_button:
    # Load model
